...
public class ChannelEndpointsTests {
    ...
    public void getChannelMembersWithoutChannel() {
        assertEquals(simple("error", "channel_not_found").toString(), ChannelEndpoints.getChannelMembers(new JSON(simple("channel", UUID.randomUUID())), UUID.randomUUID()).toString());
    }
...
////        when(this.channelHandler.notifyAllChannelUsers(ChatAction.MEMBER_JOIN, channel, users[0].getName()));
//        System.out.println(ChannelEndpoints.getChannelMembers(
...
//        final User user = new User(UUID.randomUUID(), "test", "test@test.test", new Date(), new Date());
//        when(this.microService.getUser(isA(UUID.class))).thenReturn(user);
//        assertEquals(simple("success", true).toString(), ChannelEndpoints.joinChannel(new JSON(simple("channel", channel.getUuid())), user.getUUID()).toString());
//    }
